import Link from "next/link";

const Journals = () => {
  return (
    <div className="libcontent">
      <h4>Journals </h4>
      <h3>Journals/ E-Journals/ Periodicals</h3>
      <div className="libtable">
        <table>
          <tbody>
            <tr>
              <td>DELNET On-line Resources</td>
              <td>
                Login ID: hrkrmu
                <br />
                Password: krmu5455
              </td>
              <td></td>
              <td></td>
            </tr>
            <tr>
              <td>Sr. No.</td>
              <td>Name of e-journal/ resources/ database</td>
              <td>Total No. of e-journals</td>
              <td>Login ID &amp; Password</td>
            </tr>
            <tr>
              <td>1</td>
              <td>Engineering Science &amp; Technology (IESTC)</td>
              <td>2,233</td>
              <td>
                http://infotrac.galegroup.com/itweb/krmu
                <br />
                Password: new
              </td>
            </tr>
            <tr>
              <td>2</td>
              <td>Management (IMC)</td>
              <td>3,210</td>
              <td>
                http://infotrac.galegroup.com/itweb/krmu
                <br />
                Password: new
              </td>
            </tr>
            <tr>
              <td>2</td>
              <td>Pharmacy (IPC)</td>
              <td>509</td>
              <td>
                http://infotrac.galegroup.com/itweb/krmu
                <br />
                Password: new
              </td>
            </tr>
            <tr>
              <td>4</td>
              <td>Architecture/ environmental Engineering (Greener)</td>
              <td>1,000</td>
              <td>
                http://infotrac.galegroup.com/itweb/krmu
                <br />
                Password: new
              </td>
            </tr>
            <tr>
              <td>5</td>
              <td>Manupatra (Law)</td>
              <td>0</td>
              <td>
                IP Based (No Login &amp; password required. Accessible within
                campus only)
              </td>
            </tr>
          </tbody>
        </table>

        <br />
        <h3>E-Journals</h3>
      </div>
      <div className="libtable">
        <table>
          <tbody>
            <tr>
              <td>S. No.</td>
              <td>Branch</td>
              <td>No. of Journals</td>
              <td>View &amp; Download</td>
            </tr>
            <tr>
              <td>1</td>
              <td>Chemistry</td>
              <td>15</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/Chemistry.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>2</td>
              <td>Civil Engineering</td>
              <td>15</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/Civil Engineering (OS) On-line Journals.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>3</td>
              <td>Computer Science Engineering/ IT</td>
              <td>28</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/Computer Science and Engineering (OS) On-line Journals.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>4</td>
              <td>Electronics &amp; Communication Engineering (ECE/ EEE)</td>
              <td>25</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/ECE and EEE (OS) On-line Journals.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>5</td>
              <td>Automobile Engineering</td>
              <td>16</td>
              <td>
                <Link href="javascript:void(0);">Download</Link>
              </td>
            </tr>
            <tr>
              <td>6</td>
              <td>English</td>
              <td>08</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/English.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>7</td>
              <td>Law</td>
              <td>40</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/Law Journals.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>8</td>
              <td>Journals/ E-Journals/ Periodicals and Information Science</td>
              <td>31</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/Library &amp; Information Science.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>9</td>
              <td>Management</td>
              <td>35</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/Management (OS) On-line Journals.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>10</td>
              <td>Mathematics</td>
              <td>35</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/Maths.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>11</td>
              <td>Mechanical Engineering</td>
              <td>26</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/Mechanical Engineering (OS) On-line Journals.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>12</td>
              <td>Pharmacy</td>
              <td>50</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/Pharmacy Journals.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
            <tr>
              <td>13</td>
              <td>Physics</td>
              <td>20</td>
              <td>
                <Link
                  href="https://www.krmangalam.edu.in/wp-content/uploads/2023/Journals/Physics.docx"
                  target="_blank" rel="noopener noreferrer"
                  
                >
                  Download
                </Link>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
  );
};

export default Journals;
