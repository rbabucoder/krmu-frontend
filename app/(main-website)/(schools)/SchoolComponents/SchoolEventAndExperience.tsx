...
const SchoolEventAndExperience = ({ title, desc, btn, eventsexp }: Props) => {
  return (
    <section className="bg-[url(/schools/events-bg.webp)] bg-cover bg-center py-section-sm px-4">
      <div className="max-w-main mx-auto w-full lg:flex items-center gap-5">
        ...
        <div className="lg:w-1/2 text-white">
          ...
          {(btn?.buttonclass || btn?.buttonlink) && (
            <Link
              href={btn?.buttonlink}
              className="py-3.5 px-8 bg-krmu-red-dark font-bold rounded-sm inline-block text-white mt-5"
              target="_blank"
            >
              {btn?.buttontext}
            </Link>
