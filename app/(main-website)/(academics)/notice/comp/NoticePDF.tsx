          ...
          {noticePDFS &&
            noticePDFS.map((item) => {
              return (
                <div
                  key={item?.id}
                  className="p-5 bg-[url(/academics/card-bg-1.webp)] bg-cover bg-no-repeat bg-center text-white rounded-pill flex flex-col items-center justify-center text-center"
                >
                  <p className="mb-5">{item?.content}</p>
                  <Link
                    href={item?.pdf_url || "#"}
                    target="_blank"
                    className="py-[9px] px-5 text-xs bg-krmu-red rounded-sm font-semibold"
                  >
                    View File
                  </Link>
                </div>
