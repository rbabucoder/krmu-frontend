...
const ExaminationABCDigilocker = ({ heading, url, btns }: Props) => {
  return (
    <section className="bg-krmu-navy py-5 lg:py-0 px-4 lg:pr-0 lg:pl-5">
      <div className="max-w-main mx-auto w-full lg:flex items-center gap-16">
        <div className="lg:w-2/3 break-all">
          <h3 className="text-h4 md:text-display font-medium leading-[1.7] text-white mt-2.5 mb-[15px]">
            {heading}
          </h3>
          ...
          <Link
            href={`${url || "#"}`}
            target="_blank"
            className="text-2xl text-krmu-blue mb-5 inline-block"
          >
            {url}
          </Link>
            ...
            {btns?.map((btn) => {
              return (
                <Link
                  key={btn?.id}
                  href={btn?.btn_link || "#"}
                  className={`bg-krmu-red-dark py-[5px] px-5 md:py-[13px] md:px-[29px] text-white text-xs lg:text-base font-semibold hover:bg-krmu-navy-light inline-block w-fit rounded-sm`}
                  target="_blank"
                >
                  {btn?.btn_text}
                </Link>
