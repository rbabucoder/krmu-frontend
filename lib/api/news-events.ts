...
export async function getAllNewsAndEventsWithMeta(
  ...
  const res = await fetch(
    `${FETCH_STRAPI_URL}/api/news-and-events?sort[0]=title:asc&fields[0]=title&fields[1]=slug&fields[2]=publishedAt&populate[featured_img]=true&pagination[pageSize]=${pageSize}&pagination[page]=${page}&status=published&locale[0]=en`,
    { next: { revalidate: 60 } }
  );
